...
# ---------------------------------------------------------
def run_regularization_optimization(
    run_id,
    penalty_weight,
    num_epochs,
    p_true,
    ...
    try:
        pw_str = f"{penalty_weight:.1f}"

        print(f"Worker {run_id}: Starting Penalty Weight {pw_str}")

        # Keep original boundaries for the full "big demand" (No truncation)
        new_boundaries = boundaries_base

        noise_key1, noise_key2 = jax.random.split(jax.random.PRNGKey(1127))
        new_flow = (
        ...
        # Initialize Params
        keys = jax.random.split(jax.random.PRNGKey(2443), 7)
        ...
        # Execute XLA compiled loop with dynamic penalty weight
        ...
        return {"status": "error", "penalty_weight": penalty_weight, "error": str(e)}
...
def save_convergence_plot(
    pw, loss_history, param_history, p_true, save_dir="convergence_plots"
):
    ...
    ax_loss.set_title(f"Optimization: NLL Loss (Reg: {pw:.1f})")
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, f"convergence_reg_{pw:.1f}.png")
...
if __name__ == "__main__":
    ...
    # Loop over regularization penalty weights rather than fractions
    penalty_weights = [0.0, 0.1, 0.5, 1.0, 10, 50, 100.0, 500.0, 1000.0]
    num_epochs = 2000
    ...
    pool_args = [
        ...
        for i, pw in enumerate(penalty_weights)
    ]

    print(f"\nDispatching {len(penalty_weights)} parallel tests...")

    # 1. Run optimization on workers
    with mp.Pool(processes=len(penalty_weights)) as pool:
        all_results = pool.starmap(run_regularization_optimization, pool_args)

    # 2. Process results on main thread
    print("\nProcessing results and saving individual files...")

    for result in all_results:
        pw = result["penalty_weight"]
        pw_str = f"{pw:.1f}"

        if result["status"] == "error":
            print(f"Skipping saving for penalty weight {pw_str} due to worker error.")
            continue

        # Draw and save the plot
        save_convergence_plot(
            pw, result["loss_history"], result["parameter_history"], p_true
        )
        ...
        json_path = f"id_params/identified_parameters_reg_{pw_str}.json"
