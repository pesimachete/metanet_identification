...
def generate_prbs_jax(
    ...
    """
    A 100% JAX-compatible, JIT-able PRBS generator.
    It avoids dynamic 'while' loops by pre-generating the maximum possible
    number of flips and mapping them to the time horizon using searchsorted.
    """
...
def simulate_example():
    ...
    tau = jnp.full(N, 0.08)
    nu = jnp.full(N, 50.0)
    kappa = jnp.full(N, 40.0)
    delta = jnp.full(N, 0.012)
...
def plot_trajectories(traj, dem, N: int, K: int):
    ...
    # 1. Density Plot (2D Heatmap)
    fig1, ax1 = plt.subplots(figsize=(10, 5))
    # pcolormesh replaces plot_surface for 2D representation
    ...
    # Z-label becomes the colorbar label
    fig1.colorbar(mesh1, ax=ax1, label="Density (veh/km)")

    # 2. Velocity Plot (2D Heatmap)
    ...
    fig2.colorbar(mesh2, ax=ax2, label="Speed (km/h)")

    # 3. Flow Plot (2D Heatmap)
    ...
    fig3.colorbar(mesh3, ax=ax3, label="Flow (veh/h)")

    # 4. Demand Plot (2D Heatmap)
    fig4, ax4 = plt.subplots(figsize=(10, 5))
    mesh4 = ax4.pcolormesh(time_grid, section_grid, dem.T, cmap="Reds", shading="auto")
    ax4.set_title("Demand Profile over Time and Sections")
    ax4.set_xlabel("Time Step (k)")
    ax4.set_ylabel("Section Index")
    fig4.colorbar(mesh4, ax=ax4, label="Demand (veh/h)")
